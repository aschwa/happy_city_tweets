...
if __name__ == "__main__":
    ...
    tweet_file = tweet_path
    # get city id for shape file
    city_id = tweet_path.stem[-7:]
    output_dir = Path("../../data/processed/tweets").resolve() / tweet_path.name
    # Load geodatabase, and subselect city we are looking at
    print("Will save to: {}".format(output_dir))
    ...
    try:
        ...
        pass
    tweet_geometry = tweet_df.geo.values

    # with Pool(None) as p:
    #    tweet_reproj = p.map(reproj_coords, tweet_geometry)

    tweet_reproj = [reproj_coords(xy['coordinates']) for xy in tweet_df.geo.values]
